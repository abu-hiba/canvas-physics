...
const update = (ctx: CanvasRenderingContext2D) => () => {
  ...
  const updatedCircles = state.circles.map((c) =>
    Circle.applyVelocity(
      Circle.detectCollision(canvasWidth, canvasHeight, circles, c)
    )
  );
...
const createManyCircles = (
  params: CreateManyCirclesParams,
  n: number,
  ...
  if (n > 0) {
    ...
    const collidesWithOtherCircle = !!circles.find(
      Circle.circleCollision(newCircle)
    );
...
window.onload = () => {
  ...
  if (canvas && context) {
    ...
    const initialCircles = createManyCircles(
      { context, minRadius, maxRadius, maxSpeed },
      numCircles
    );
