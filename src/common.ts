...
export const generateRandomPosition = (canvasWidth: number, canvasHeight: number, radius: number): Point => ({
...
export const generateRandomVelocity = (maxSpeed: number): Velocity => ({
  x: Math.random() * (2 * maxSpeed) - maxSpeed, // random number between -maxSpeed and maxSpeed
  y: Math.random() * (2 * maxSpeed) - maxSpeed
})

export const dotProduct = (v1: Vector, v2: Vector): number => v1.x * v2.x + v1.y * v2.y

export const vectorNorm = ({ x, y }: Vector): number => Math.sqrt(x**2 + y**2)
...
export const subtractVector = (v1: Vector, v2: Vector): Vector => ({
  x: v1.x - v2.x,
  y: v1.y - v2.y
})
